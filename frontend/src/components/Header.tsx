        ...
        <div className="text-center text-lg font-medium flex items-center gap-4 content-center p-4">
            ...
            <nav className="flex gap-6  ml-auto">
                ...
                {isInstructor ? (
                    <Link 
                        to="/create-project" 
                        className={`hover:text-gray-300 ${location.pathname === '/create-project' ? 'text-blue-500' : ''}`}
                    >
                        Створити проект
                    </Link>
                ) : (
                    <Link 
                        to="/my-submissions" 
                        className={`hover:text-gray-300 ${location.pathname === '/my-submissions' ? 'text-blue-500' : ''}`}
                    >
                        Мої заявки
                    </Link>
                )}
